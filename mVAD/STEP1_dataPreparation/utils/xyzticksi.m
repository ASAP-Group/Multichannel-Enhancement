...
%      Version: $Id: xyzticksi.m 2011-10-14:45:43Z dmb $
%
...
% Bugs:
%   (1) ipan=or is not really debugged yet:
...
marg=[0.0.0.25];     % gap between labels in font units
ntreq=[1];        % minimum number of labelled ticks required as a function of IPAN
% grid template: each pair is [#steps final-value]. Start=10, end=100
lgridtem={1; [1]; [4]; 9; [8]; [7]; [5]};
...
if ax==1
    ...
    axdir=[0];        % unit vector in axis direction
else
    widthc=2*getgcac.Position(4)/getgcac.FontSize;     % y axis length in font units
        axdir=[1];        % unit vector in axis direction
end
...
if strcmp(getgca.(axfield{5,ax}),'log')   % log axis
    ...
    if ai3>=-&& bi3<=             % do nothing if outside the SI multiplier range
        ...
        ai3=3*ceil(ai/3);
        bi3=3*floor(bi/3);
        for ipan=1:                   % panic level: 1=spacious, 2=cramped, 3=any two labels, 4=any label
            ...
            if incsi==        % no point in trying anything else if incsi>3
                ...
                for jgrid=1:ngrid
                    ...
                    widq=widq(msk);
                    logvq=logvq(msk);
                    % debug1=[intq(msk); -min((widq(1:end-1)+widq(2:end)+2*margin)/axdir(1),2*(digith+margin)/axdir(2))<=2*width10*(logvq(2:end)-logvq(1:end-1))];
                ...
                if ntick<5
                    ...
                    while 1
                        ...
                        % debug2=[intq; widq; -(widq(1:end-1)+widq(2:end)+2*margin)<=2*width10*(logvq(2:end)-logvq(1:end-1))];
    ...
    if ntick
        ...
        if dosubtick       % check for subticks
            subtick=[];
            if ntick>     % at least two labelled ticks
                ...
                substep=[--5]*((1+[1; 2; 5]*(subbase./stepleft))>ratthresh); % actual step is 1,or times subbase
                substep(stepint~=10*substep)=max(2-substep(stepint~=10*substep),0); % but only >if stepint==10
    ...
    for ipan=1:                   % panic level: 1=spacious, 2=cramped, 3=any two labels, 4=any label
        ...
        while % loop trying increasingly coarse labelling
            ...
            if all((widq(1:end-1)+widq(2:end)+2*margin)<=2*axdir(1)*width1*delta) || (digith+margin<=axdir(2)*width1*delta);
                ...
                tickdp=dpq;
                ticksi=siq;
                if deltav>&& width1*delta>0.5*deltav          % put explicit subticks if delta = or 5
