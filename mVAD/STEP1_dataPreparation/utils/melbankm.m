...
%              'y' = lowest filter remains at down to frequency and
%                    highest filter remains at up to nyquist freqency
%
...
%                 Note that the peak filter values equal to account for the power
...
%     IEEE Trans Acoustics Speech and Signal Processing, (4): 357–366, Aug. 1980.


%      Copyright (C) Mike Brookes 1997-2009
%      Version: $Id: melbankm.m 2011-10-14:45:43Z dmb $
%
...
%   Free Software Foundation, Inc.,Mass Ave, Cambridge, MA 02139, USA.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Note "FFT bin_0" assumes DC = bin whereas "FFT bin_1" means DC = bin 1
...
sfact=2-any(w=='s');   % if single sided else 2
...
if ~any(w=='H')
    switch wr
        ...
        case 'l'
            if fl<=0
                error('Low frequency limit must be >for l option');
            end
            mflh=log10(mflh);       % convert frequency limits into logHz
        case 'e'
...
switch wr
        case 'f'
        blim=(mflh(1)+[p p+1]*melinc)*n/fs;
    case 'l'
        blim=10.^(mflh(1)+[p p+1]*melinc)*n/fs;
    case 'e'
        blim=erb2frq(mflh(1)+[p p+1]*melinc)*n/fs;
    case 'b'
        blim=bark2frq(mflh(1)+[p p+1]*melinc)*n/fs;
    otherwise
        blim=mel2frq(mflh(1)+[p p+1]*melinc)*n/fs;
end
mc=mflh(1)+(1:p)*melinc;    % mel centre frequencies
b1=floor(blim(1))+1;            % lowest FFT bin_required might be negative)
b4=min(fn2,ceil(blim(4))-1);    % highest FFT bin_required
%
% now map all the useful FFT bins_to filtercentres
%
switch wr
        case 'f'
...
fp=floor(pf);                  % FFT bin_i contributes to filters_fp(1+i-b1)+[1]
pm=pf-fp;                       % multiplier for upper filter
k2=find(fp>0,1);   % FFT bin_k2+bis the first to contribute to both upper and lower filters
k3=find(fp<p,1,'last');  % FFT bin_k3+bis the last to contribute to both upper and lower filters
k4=numel(fp); % FFT bin_k4+bis the last to contribute to any filters
if isempty(k2)
    k2=k4+1;
end
if isempty(k3)
...
if sfact== % double all except the DC and Nyquist (if any) terms
...
if nargout > 2
    x=sparse(r,c,v);
    if nargout ==     % if exactly three output arguments, then
...
if ~nargout || any(w=='g') % plot idealized filters
    ...
    v=v*sfact;  % multiply by if double sided
    v=repmat(v,p,1);
